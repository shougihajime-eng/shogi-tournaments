...
- ✅ 直近で済んだこと:
  - 💰賞金大会バッジ＆固定セクション追加（アマ竜王戦・都名人戦・FUYOU杯茨城など自動検出）
  - 統計バナーに「賞金大会」追加、クリックで /?prize=1 絞り込み
  - 本番公開（https://shogi-tournaments.vercel.app）
  - 大会一覧の月別グルーピング・トップへ戻るボタン追加
  - スマホでのタップしやすさ改善（最小44px）
  - キーボードショートカット・印刷対応・SEO
- 🟡 進行中: なし
- 🔜 次の一歩: 未定（茨城50万円大会の正体特定 / 賞金ラベル精度の改善 など）

## 賞金大会の判定
`lib/filters/prize.ts` で大会タイトル＋説明文をキーワード判定。DB変更なし、毎日のスクレイピング後に既存データへも自動反映される。新しい賞金大会名を追加する場合は `PRIZE_RULES` 配列を編集。
